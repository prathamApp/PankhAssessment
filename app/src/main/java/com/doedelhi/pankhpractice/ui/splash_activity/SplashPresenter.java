...
import com.doedelhi.pankhpractice.services.LocationService;
...
import com.doedelhi.pankhpractice.utilities.SDCardUtil;
...
import static com.doedelhi.pankhpractice.ui.splash_activity.SplashActivity.appDatabase;
...
//            getSdCardPath();
            requestLocation();
            AssessmentApplication.sharedPreferences.edit().putBoolean(Assessment_Constants.INITIAL_ENTRIES, true).apply();
...
//        updateNewEntriesInStatusTable();
        if (!AssessmentApplication.sharedPreferences.getBoolean(Assessment_Constants.SD_CARD_Content_STR, false)) {
            ...
            try {
                File db_file;
                db_file = new File(Environment.getExternalStorageDirectory().toString() + "/.PankhAssessmentInternal/" + AppDatabase.DB_NAME);
                if (db_file.exists()) {
                    Assessment_Constants.SD_CARD_Content = true;
